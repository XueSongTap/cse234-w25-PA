...
class Node:
    ...
    node：图里的节点（张量/中间量/输出），每个节点记录：

        inputs：这个节点依赖的前驱节点们

        op：本节点执行的算子类型（加法、乘法、Softmax…）

        attrs：算子需要的额外参数（如常数、axis、shape等）
    ...
    """
        # 这里把 Python 的 + - * / 语法“重载”为建图：
        # 遇到 Node ⊕ Node → 调用对应 Op 实例（如 add/mul/div）
        # 遇到 Node ⊕ 常数 → 调用 *_by_const 的 Op 实例
    """
...
class Op:
    ...
    Op：算子的抽象接口，规定 3 件事：
        ...
        __call__：建图（创建 Node，填好 inputs/op/attrs/name）

        compute：前向（给定输入张量值，算当前节点的张量值）

        gradient：反向（给定“本节点的上游梯度节点”，构造对每个输入的梯度子图，返回“梯度节点列表”）
    """
...
class GreaterThanOp(Op):
    ...
    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        ...
        (A > B) 是离散、不可导的操作；几乎处处对输入的导数为 0（在 A==B 的集合也没良好导数）。
        因而这里选择梯度为 0（这是常见做法，相当于把比较视为“阻断梯度”的节点）。
        返回的是梯度节点而不是数值：zeros_like(x) 会在求值时生成与 x 同形状的全 0 张量，所以梯度形状匹配。        
        """
...
class BroadcastOp(Op):
    """
    把一个张量按 PyTorch 的广播规则“扩到目标形状”，并在反向把梯度沿被广播的轴求和还原回输入形状
    """
...
class MatMulOp(Op):
    """Matrix multiplication op of two nodes.矩阵乘"""
...
class LayerNormOp(Op):
    ...
    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        ...
        # 归一化维度（最后 len(norm_shape) 个维度）
        # 注意：在“建图”时我们无法访问实际张量维度，所以这里用与 compute 中一致的规则表达：
        # 用负索引描述：最后 len(norm_shape) 个维度为 dims = (-len(norm_shape), ..., -1)
        r = len(norm_shape)
        # 构造一个以负索引表示的 dim 元组，Mean/ Sum 的 __call__ 接受 tuple
        dims = tuple(range(-r, 0)) if r > 0 else ()

        # 计算 mean 和 var（无偏置：unbiased=False）
        x_mean = mean(x, dim=dims, keepdim=True)
        xc = x - x_mean
        var = mean(xc * xc, dim=dims, keepdim=True)  # 无偏 = E[(x-mean)^2]
...
class Evaluator:
    ...
    Evaluator：执行器。给它一批待求值的节点（可以包含前向结果和梯度节点），加上输入变量的数值，它会做拓扑排序并依次调用 op.compute，把这些节点的数值都算出来。
    """
